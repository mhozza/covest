                ...
                for k in ks:
                    ...
                    if compute_hist:
                        if use_jellyfish:
                            ...
                            run(khmer_count.format(**params))
                            run(khmer_hist.format(**params))
                    run(estimator.format(**params),
                        '{infile_base}_k{k}.est.out'.format(**params))
                    if run_khmer:
                        run(khmer_cov.format(**params),
                            '{infile_base}_k{k}.khmer.out'.format(**params))
